...
func TestAccountProtectionOrdinaryUpdateCannotDisableOrErasePolicy(t *testing.T) {
	...
	for _, extra := range []map[string]any{{}, {AntiDegradationExtraKey: false}, {AntiDegradeMarkerExtraKey: nil}} {
...
func TestGenericProtectionRevertPreservesUnmanagedTLS(t *testing.T) {
	ctx := context.Background()
	parent := int64(7)
	a := &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Concurrency: 32, ParentAccountID: &parent, QuotaDimension: "spark",
	...
	admin := &adminServiceImpl{accountRepo: &upstreamBillingProbeAccountRepo{accounts: map[int64]*Account{1: a}}}
	svc := NewAntiDegradeService(admin)
	_, err := svc.ApplyAntiDegradeMode(ctx, 1, AntiDegradeMode1)
	require.Error(t, err, "影子账号只支持通用并发保护")
	restored, err := svc.Revert(ctx, 1)
	require.NoError(t, err)
	require.False(t, restored.AntiDegradationEnabled())
	require.True(t, restored.IsTLSFingerprintEnabled())
	require.EqualValues(t, 9, restored.GetTLSFingerprintProfileID())
	reenabled, err := svc.SetProtection(ctx, 1, true, false)
	require.NoError(t, err)
	require.Equal(t, "generic", reenabled.ProtectionMode())
}
