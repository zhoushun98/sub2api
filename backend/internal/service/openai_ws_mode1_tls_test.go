...
func TestMode1WSConnectionPoolTransportCompatibility(t *testing.T) {
	...
	defer cancel()
	first, err := pool.acquire(ctx, cloneOpenAIWSAcquireRequest(req), 0, &openAIWSAcquireQueueWait{})
	require.NoError(t, err)
	firstID := first.ConnID()
	first.Release()
	reused, err := pool.acquire(ctx, cloneOpenAIWSAcquireRequest(req), 0, &openAIWSAcquireQueueWait{})
	require.NoError(t, err)
	...
	rotated, err := pool.acquire(ctx, cloneOpenAIWSAcquireRequest(req), 0, &openAIWSAcquireQueueWait{})
	require.NoError(t, err)
	require.NotEqual(t, firstID, rotated.ConnID())
	require.False(t, rotated.Reused())
	rotated.Release()
	req.WSURL += "/other"
	otherTarget, err := pool.acquire(ctx, cloneOpenAIWSAcquireRequest(req), 0, &openAIWSAcquireQueueWait{})
	require.NoError(t, err)
	...
	other, err := pool.acquire(ctx, cloneOpenAIWSAcquireRequest(req), 0, &openAIWSAcquireQueueWait{})
	require.NoError(t, err)
