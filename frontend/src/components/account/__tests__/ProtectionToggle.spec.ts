...
describe('ProtectionToggle', () => {
  ...
  it('discloses the generic capability boundary and does not mislabel it as Codex', () => {
    const wrapper = shallowMount(ProtectionToggle, { props: { account: account() } })
    expect(wrapper.get('button').attributes('title')).toBe('admin.accounts.protectionScopeGeneric')
    expect(wrapper.text()).toContain('admin.accounts.protectionGenericLabel admin.accounts.protectionOn')
  })
