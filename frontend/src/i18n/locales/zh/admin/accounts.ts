export default {
    accounts: {
      ...
      antiDegrade: '账号保护配置',
      antiDegradeDesc: '稳定账号设备身份、隔离不同会话并限制突发并发；允许已知协议转换，阻止意外丢失输入、工具或推理内容。应用前可预览，应用后可还原。保护策略不保证上游模型质量。',
      ...
      antiDegradeGenericScope: '通用保护（并发上限）',
      ...
      antiDegradeEffectiveTLS: '当前有效传输',
      antiDegradeEffectiveConcurrency: '有效并发上限',
      antiDegradeObserved: '已有传输观测',
      antiDegradeComputed: '以上为配置计算结果，尚无握手观测',
      antiDegradeMode1Desc: '适用于 OpenAI OAuth / setup-token。v3 使用标准传输，不强制 TLS 模板；保留账号身份种子、独立会话和并发上限。OAuth 不支持 max_output_tokens，该字段会按接口能力移除。',
      antiDegradeMode2Desc: '模式二保留旧方案，仅用于对照。可预览原有配置或还原，不包含模式一的保证。',
      antiDegradeApply: '应用',
      antiDegradeMode1: '模式一：兼容保护 v3',
      antiDegradeMode2: '模式二：完整收敛',
      antiDegradeModeLegacy: '初代兼容',
      ...
      antiDegradeDefaultTag: '默认',
      antiDegradeShowDiagnostics: '显示诊断策略',
      antiDegradeHideDiagnostics: '收起诊断策略',
      ...
      antiDegradeImmediateHint: '保护开关、策略应用与还原会立即生效；其他字段通过底部保存。诊断策略用于对照排查。',
      antiDegradeDisableTitle: '关闭账号保护？',
      antiDegradeDisableMessage: '关闭会立即还原该策略管理的身份与传输设置。管理员配置的并发数保持不变。尚未保存的编辑会保留在当前表单中。',
      antiDegradeChangeStrategy: '生效策略',
      antiDegradeChangeFingerprint: '账号身份策略',
      antiDegradeChangeTLS: '固定 TLS 握手特征',
      antiDegradeChangeConcurrency: '设置初始并发（可独立修改）',
      requestIntegrity: '请求完整性检查',
      requestIntegrityHint: '独立于身份与传输策略。跟随策略默认：初代兼容仅观察，模式一严格拦截。',
      requestIntegrityDefault: '跟随策略默认',
      requestIntegrityOff: '关闭',
      requestIntegrityObserve: '仅观察（记录差异）',
      requestIntegrityEnforce: '严格拦截',
      protectionLabel: '账号保护',
      protectionGenericLabel: '并发保护',
      protectionOn: '已开启',
      protectionOff: '已关闭',
      protectionToggleOn: '开启账号保护',
      protectionToggleOff: '关闭账号保护',
      protectionToggleConfirmMessage: '关闭后将还原该策略管理的身份、传输或并发设置。保护策略不保证模型答题质量。',
      protectionToggleConfirm: '确认关闭',
      protectionEnabledToast: '账号保护已开启',
      protectionDisabledToast: '账号保护已关闭',
      protectionUpdateFailed: '更新账号保护设置失败',
      protectionScopeDisabled: '管理员已关闭保护',
      protectionScopeGeneric: '通用保护：并发上限。此账号暂不支持 Codex 专属身份与请求完整性检查，不保证上游模型质量。',
      protectionScopeCodex: '兼容保护 v3：稳定设备身份、会话隔离、并发上限及请求完整性；不保证上游模型质量。',
      protectionScopeSaved: '沿用已保存的保护策略；查看账号配置可了解具体能力范围',
      protectionBatchEnable: '开启保护',
      protectionBatchSuccess: '已为 {count} 个账号开启防降智保护',
      protectionBatchPartial: '已开启 {success} 个账号，{failed} 个失败，请刷新后检查',
      protectionBatchFailed: '批量开启失败，请重试',
      title: '账号管理',
